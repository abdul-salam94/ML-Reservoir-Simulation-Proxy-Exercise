...
class ReservoirPreprocessor:
    ...
    def check_existing_data(self):
        ...
        if not sys.stdin.isatty():
            # Patched: prefer "use existing" so crash recovery (e.g., re-run after
            # OOM in Step 3) reuses the completed Steps 1-2 instead of redoing them.
            logger.info(
                "\nNon-interactive environment detected. Auto-selecting 'n' (use existing, run only missing steps)"
            )
            return False
